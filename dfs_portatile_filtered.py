...
# I metodi sono gli stessi del file dfs_portatile ma viene applicato per trovare i path nei
# dataframe filtrati. Quindi:
# dfs_portatile --> WN18/FB15k
# dfs_portatile_filtered --> WN18RR/FB15k-237

def creaInversi():
    with open("data/WN18RR/train_WN18RR.csv") as f:
        lines = f.readlines()
    CSVinv = open("data/WN18RR/train_WN18RR-inv.csv", 'w')
...
def partenza_dfs(head,tail,df,DEEPdfs,DEEPreversedfs):
    if(not os.path.isfile("data/WN18RR/train_WN18RR-inv.csv")):
        creaInversi()
    df=pd.read_csv('data/WN18RR/train_WN18RR-inv.csv', sep='\t', names=['head', 'relation', 'tail'])
    start = ('', '', head)
    stop = ('', '', tail)
    paths = []
    visitati = []
